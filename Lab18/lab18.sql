...
-- PARTE 1:
-- La suma de las cantidades e importe total de todas
-- las entregas realizadas durante el 97.
SET DATEFORMAT dmy
SELECT
  m.Clave,
...
-- Para cada proveedor, obtener la razón social del
-- proveedor, número de entregas e importe total de las entregas realizadas.
SELECT
  p.RazonSocial,
...
-- Por cada material obtener la clave y descripción
-- del material, la cantidad total entregada, la mínima
-- cantidad entregada, la máxima cantidad entregada,
-- el importe total de las entregas de aquellos materiales
...
-- Para cada proveedor, indicar su razón social y mostrar
-- la cantidad promedio de cada material entregado,
...
-- la consulta anterior pero para dos grupos de
-- proveedores: aquellos para los que la cantidad
-- promedio entregada es menor a 370 y aquellos para los
-- que la cantidad promedio entregada sea mayor a 450.
SELECT
  p.RazonSocial,
  AVG(e.Cantidad) AS "Cantidad Promedio",
  m.Clave,
...
-- Considerando que los valores de tipos CHAR y VARCHAR deben ir
-- encerrados entre apóstrofes, los valores numéricos se escriben
-- directamente y los de fecha, como '1-JAN-00' para 1o. de
